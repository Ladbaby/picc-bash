...
- curl -s url | jq '.data[]' \u2192 "Fetch JSON from URL and extract data array elements"`;
	...
	// Long-form Bash instructions, verbatim from
	// claude-code/tools/BashTool/prompt.ts `getSimplePrompt()` (line 354+),
	// including the commit/PR sections. Surfaced via `description`.
	const BASH_TOOL_DESCRIPTION = `Executes a given bash command and returns its output.
...
🤖 Generated with [Pi Agent](https://pi.dev/)
EOF
)"
</example>
...
- View comments on a Github PR: gh api repos/foo/bar/pulls/123/comments`;
	...
	pi.registerTool({
		...
		promptGuidelines: [BASH_PROMPT_GUIDELINE],
